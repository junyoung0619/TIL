...
public class ExceptionTest1 {
	public static void main(String[] args) {
		...
		// 2. 예외가 발생했는데 성공적으로 처리되지 못했을 때(catch 문을 못 만났을 때) 1 (2-비정상적종료)
		// 3. 예외가 발생하지 않았을 때: 1 2 3 5
		try {
			System.out.println("정상코드 1"); // (1) 정상코드
			System.out.println(nums[0]);   // (2) 예외 발생 가능한 코드
			System.out.println("정상코드 2");  // (3) 정상코드
		} catch (ArithmeticException e){ // try문에서 발생한 예외가 이 catch문과 만날 수 있는 경우는 
